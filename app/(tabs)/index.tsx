...
const getFallbackValues = (error: Error | null, movies: any) => {
  if (error) {
    return [
      {
        adult: false,
...
export default function Index() {
  ...
  const moviesToRender = getFallbackValues(error, movies);
    ...
    <View className="flex-1 bg-primary">
      ...
      >
        ...
        {loading || trendingLoading ? (
          ...
          />
        ) : (error || trendingError) && !moviesToRender ? (
          <Text className="text-white">
            Error: {(error || trendingError || {}).message}
          </Text>
        ) : (
